...
import 'package:assignment_6_arch_pattern/model/CityItem.dart';
import 'package:assignment_6_arch_pattern/model/WeatherData.dart';
import 'package:assignment_6_arch_pattern/model/WeatherDetails.dart';
...
class WeatherDataService {
  ...
  WeatherData convertResultInWeatherData(WeatherDetails weatherDetails) {
    String temp =
        convertKelvinToCelcius(weatherDetails.main.temp).toStringAsFixed(2);
    String tempMin =
        convertKelvinToCelcius(weatherDetails.main.tempMin).toStringAsFixed(2);
    String tempMax =
        convertKelvinToCelcius(weatherDetails.main.tempMax).toStringAsFixed(2);
    String status = weatherDetails.weather[0].main;
    String city = weatherDetails.name;
    String country = weatherDetails.sys.country;

    return WeatherData.weatherDetails(temp + DEGREE_SYMBOL,
        "${tempMin} - ${tempMax} ${DEGREE_SYMBOL}", status, city, country);
  }
